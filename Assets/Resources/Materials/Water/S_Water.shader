Shader "KJK/S_Water"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            CGPROGRAM
            #pragma surface surf MY vertex:vert addshadow keepalpha
            ...
            v2f vert(inout appdata_full v)
            {
                ...
                float wave;
                wave = cos(abs(4 * o.uv.x - 2) * 3 + _Time.y * _waveSpeed) * _wavePower;
                wave += cos(abs(2 * o.uv.y - 1) * 10 + _Time.y * _waveSpeed) * _wavePower;
                wave += sin(abs(2 * o.uv.x - 1) * 5 + _Time.y * _waveSpeed) * _wavePower;
                if (v.color.r >= 0.5)
                    v.vertex.y = wave * 0.5;
                else
                    v.vertex.y = wave;
