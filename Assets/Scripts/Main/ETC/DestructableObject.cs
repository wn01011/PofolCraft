...
public class DestructableObject : MonoBehaviour
{
    ...
    private AudioClip destructSound = null;
    ...
    {
        destructSound = SoundManager.Instance.WoodBreak;
        AudioSource audioSource = gameObject.AddComponent<AudioSource>();
        audioSource.maxDistance = 20f;
        audioSource.spatialBlend = 1f;
        audioSource.volume *= SoundManager.Instance.SFXVolume;
        audioSource.PlayOneShot(destructSound, audioSource.volume);
    }
    ...
    public void DestroyMesh()
    {
        ...
        if (originalMesh.vertexCount <= 4) return;
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            {
                if(!parts[i].GameObject.GetComponent<Rigidbody>())
                    parts[i].GameObject.AddComponent<Rigidbody>();
                parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * ExplodeForce, transform.position);
                StartCoroutine(DestoryParts(parts[i].GameObject));
            }
    ...
    private IEnumerator DestoryParts(GameObject _parts)
    {
        Destroy(_parts.GetComponent<DestructableObject>());
        _parts.layer = 2; // IgnoreRaycast;
        float timer = 0f;


        Color _partsColor = _parts.GetComponent<MeshRenderer>().material.color;
        _parts.GetComponent<MeshRenderer>().material.color = new Color(_partsColor.r, _partsColor.g, _partsColor.b, 1.0f);

        while (timer <= 3.0f)
        {
            timer += Time.deltaTime;
            _partsColor = _parts.GetComponent<MeshRenderer>().material.color;
            _parts.GetComponent<MeshRenderer>().material.color = new Color(_partsColor.r, _partsColor.g, _partsColor.b, 1.0f - (timer / 3.0f));
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(DestructableObject original)
        {
            if (original.GetComponent<MeshFilter>().mesh.vertexCount <= 10 || !original.GetComponent<MeshRenderer>()) return;
            ...
            if (collider.sharedMesh.vertices.Length >= 4)
            {
                ...
                var meshDestroy = GameObject.AddComponent<DestructableObject>();
